...
{
	struct cpu_ctx ctx;
    ctx.eip = (unsigned int)ip;
    ctx.eax = (unsigned int)arg;
    ctx.eflags = 0x0002;
	ctx.ebx = 0;
	ctx.ecx = 0;
	ctx.edx = 0;
...
int kfork(struct cpu_ctx ctx)
{
    ...
    // Its 0 at this point.
    // Copy memory into this process.
    proc->p_exec = (void *)kmalloc(current_process->p_exec_size);
    memcpy(proc->p_exec, 
    ...
    if(proc->p_delete_argv) {
        ...
        proc->p_argv = (char **)kmalloc(sizeof(char *) * proc->p_argc);
        for(i = 0; i < proc->p_argv; ++i) {
            proc->p_argv[i] = (char *)kmalloc(
