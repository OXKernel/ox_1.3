...
static
irq_stat_t pit_net_thread_handler(int irq)
{
    //printk("socket_timer: \n");
// FIXME    socket_timer(); // Must call LWIP threads. These are created by sys_thread_new.
    extern irq_stat_t isr_wrapper(int irq);
    isr_wrapper(5);
    socket_timer(); // RGD
    return IRQ_ENABLE;
}
...
#define _TEST_PIT
    ...
    // state.
    // TODO: Note the PIT_DELAYCALIB is being used to launch 
    // LWIP threads. We should combine them into one callback.
    // For threads and delay calibration.
    pit_install_handler(PIT_DELAYCALIB, pit_net_thread_handler);
    pit_enable();
    asm_enable_interrupt();
#endif
