...
*----------------------------------------*/
//#define Base_ADDR           0x0c100 ///CE2 space of DSK is 0xA0000000
//#define Base_ADDR           0x0c000 ///CE2 space of DSK is 0xA0000000
#define Base_ADDR           0x300 ///CE2 space of DSK is 0xA0000000 RGD
									 // and ethernet chip is at 0x200 by default

// actual address on DSK
#define     EN_CMD          (Base_ADDR+0x00)  /*	The command register (for all pages) */
#define     EN_DATA		    *(unsigned short *)(Base_ADDR+0x10)	/*by ming (change to 16bit)  Remote DMA Port10~17h (for all pages)*/
#define     EN_RESET	    (Base_ADDR+0x1F)	/*  Reset Port 1fh(for all pages)     */

/* Page 0 register offsets   */
#define     EN0_STARTPG     (Base_ADDR+0x01)	/*  WR Starting page of ring buffer      */
#define     EN0_STOPPG  	(Base_ADDR+0x02)	/*  WR Ending page +1 of ring buffer     */
#define     EN0_BOUNDARY	(Base_ADDR+0x03)	/*  RD/WR Boundary page of ring buffer   */
#define     EN0_TSR			(Base_ADDR+0x04)	/*  RD Transmit status reg               */
#define     EN0_TPSR		(Base_ADDR+0x04)	/*  WR Transmit starting page            */
#define     EN0_NCR			(Base_ADDR+0x05)	/*  RD Number of collision reg           */
#define     EN0_TCNTLO  	(Base_ADDR+0x05)	/*  WR Low  byte of tx byte count        */
#define     EN0_CRP			(Base_ADDR+0x06)	/*  Current Page Register                              */
#define     EN0_TCNTHI		(Base_ADDR+0x06)	/*  WR High byte of tx byte count        */
#define     EN0_ISR			(Base_ADDR+0x07)	/*  RD/WR Interrupt status reg           */
#define     EN0_CRDALO  	(Base_ADDR+0x08)	/*  RD low byte of current remote dma add*/
#define     EN0_RSARLO		(Base_ADDR+0x08)	/*  WR Remote start address reg 0        */
#define     EN0_CRDAHI		(Base_ADDR+0x09)	/*  RD high byte, current remote dma add.*/
#define     EN0_RSARHI		(Base_ADDR+0x09)	/*  WR Remote start address reg 1        */
#define     EN0_RCNTLO	    (Base_ADDR+0x0A)	/*  WR Remote byte count reg 0           */
#define     EN0_RCNTHI		(Base_ADDR+0x0B)	/*  WR Remote byte count reg 1           */
#define     EN0_RSR			(Base_ADDR+0x0C)	/*  RD RX status reg                     */
#define     EN0_RXCR		(Base_ADDR+0x0C)	/*  WR RX configuration reg              */
#define     EN0_TXCR		(Base_ADDR+0x0D)	/*  WR TX configuration reg              */
#define     EN0_DCFG		(Base_ADDR+0x0E)	/*  WR Data configuration reg            */
#define     EN0_IMR			(Base_ADDR+0x0F)	/*  WR Interrupt mask reg                */

/* Page 1 register offsets    */
#define     EN1_PAR0	    (Base_ADDR+0x01)	/* RD/WR This board's physical ethernet addr */
#define     EN1_PAR1	    (Base_ADDR+0x02)
#define     EN1_PAR2	    (Base_ADDR+0x03)
#define     EN1_PAR3	    (Base_ADDR+0x04)
#define     EN1_PAR4	    (Base_ADDR+0x05)
#define     EN1_PAR5	    (Base_ADDR+0x06)
#define     EN1_CURR	    (Base_ADDR+0x07)   /* RD/WR current page reg */
#define		EN1_CURPAG		EN1_CURR
#define     EN1_MAR0        (Base_ADDR+0x08)   /* RD/WR Multicast filter mask array (8 bytes) */
#define     EN1_MAR1		(Base_ADDR+0x09)
#define     EN1_MAR2        (Base_ADDR+0x0A)
#define     EN1_MAR3        (Base_ADDR+0x0B)
#define     EN1_MAR4        (Base_ADDR+0x0C)
#define     EN1_MAR5        (Base_ADDR+0x0D)
#define     EN1_MAR6        (Base_ADDR+0x0E)
#define     EN1_MAR7        (Base_ADDR+0x0F)
